...
// ModelLibraryPanel —— ProviderEditModal 内嵌的"模型库"面板 (仿 Locus / PlotCraft 同款)
//
// 视觉 + 交互镜像 PlotCraft `ModelLibraryPanel.vue`:
// - 标题 + 展开/收起 toggle + 总数提示
// - 搜索框 (按 provider name / endpoint / model id / model name 过滤)
...
//
// v0.2+ 数据源从硬编码 BUILTIN_MODELS 改成 Tauri models.dev catalog
...
<template>
  <div class="model-library" :class="{ collapsed: !expanded, loading: loading }">
    ...
    <div class="library-header">
      ...
      >
        <span class="library-toggle-arrow">{{ expanded ? "▾" : "▸" }}</span>
    ...
    <div v-if="expanded" class="library-content">
      ...
      <p v-if="!search && !error" class="library-hint">
        v0.2 内置 models.dev snapshot — 全部 {{ catalog?.providers.length ?? 0 }} 个
      ...
      <div v-if="loading && !catalog" class="library-status">加载 catalog…</div>
